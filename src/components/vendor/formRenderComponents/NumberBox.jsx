...
export default class NumberBox extends Component {
    ...
    render() {
        ...
        const disabled = ((this.props.readOnly === true) || (this.props.presetValue === true)) ? "disabled" : "";
        return (<div className={"numberControl form-group " + (this.props.hidden ? 'hidden' : 'shown')} id={this.props.name + 'Div'} style={this.props.controlStyle}>
                <label>{this.props.label}{this.isRequired()}</label>
                {this.renderDescription()}
                <span className="forComment"></span>
            <div>
                <input id={inputId} disabled={disabled} className="form-control" type="number" min={this.props.min} max={this.props.max} onKeyPress={(evt) => this.props.checkDigitsNo(evt, this.props.value, this.props.max, this.props.index)} onBlur={(evt) => this.lostFocusNumberCheck(evt, this.props.max, this.props.min)}
                    step={this.props.step} value={this.props.value == null ? '' : this.props.value} onChange={(evt) => this.props.handleInput(evt.target.value, this.props.index)} placeholder={this.props.placeholder} />
                ...
                <div className="printNumber" style={{ display: 'none' }}>{this.props.value}</div>
            </div>
            </div>);
    }
    ...
    lostFocusNumberCheck(event, max, min) {
        if (event.target.value < parseInt(min) || event.target.value > parseInt(max)) {
            event.target.value = ''
            this.props.value=''
        }
    }
