...
class FormRender extends React.Component {
    ...
    getData = () => {
        ...
        this.state.formData.forEach((data) => {
            ...
            if (dataCopy.type === 'date')
                dataCopy = this.getDateTimeValue(dataCopy);
                ...
                if (formData[i].type == "grid2") {
                    for (let j = 0; j < formData[i].value.data.length; j++) {
                        for (let k = 0; k < formData[i].value.data[j].length; k++) {
                            ...
                            if (typeof formData[i].value.data[j][k].value.value === 'string') {
                                if (formData[i].value.data[j][k].value.type === "number") {
                                    formData[i].value.data[j][k].value.value = formData[i].value.data[j][k].value.value.replace(/[\/\~:?]/g, '');
                                } else {
                                    formData[i].value.data[j][k].value.value = formData[i].value.data[j][k].value.value.replace(/[\/\~.:?]/g, '');
                                }
    ...
    getDateTimeValue(dataCopy) {
        ...
        if (val && val != '') {
            dataCopy.value = moment(val, dataCopy.format).format(valueFormat);
        ...
        formData.every((element, index) => {
            ...
            try {
                if (priorityObject.value) {
                    if (priorityObject.value.value === "High" && element.label === "Business Continuity Plan & Process (BCPP)") {
                        if (element.required === true) {
                            if (this.checkValue(element)) {
                                return true;
                            } else {
                                toastr.error("".concat(element.label, userLang != "fr" ? " is required" : " est obligatoire"));
                                flag = false;
                                return false;
                            }
                        } else return true;
            ...
            // Do your thing, then:
            if (element.required === true && ((element.conditioanalValue && !($("#" + element.name + "Div").is(':hidden'))) || !element.conditioanalValue) ) {
    ...
    removeFile = (fileObj, fieldName) => {
        ...
        var url = window.location.origin + '/ActionCentre/DeleteEvidenceFile/?fileId=' + fileObj.fileId + '&deleteFile=' + fileObj.deleteUrl;
        fetch(url, {
            method: 'GET'
        }).then((response) => {
                ...
                for (let i = 0; i < formData.length; i++) {
                    if (formData[i].name === fieldName) {
                        ...
                        for (let j = 0; j < formData[i].value.length; j++) {
                            if (formData[i].value[j].fileId !== responseData.deletedFileId) {
    ...
    handleChange = (val, index) => {
        ...
        let obj = formData[index];
        let isValidValue = (obj.type == 'text' || obj.type == 'textarea') ?
            /[{}]/.test(val) ?false:true: true;
        if (isValidValue) {
            ...
            if (this.props.selectedControl) {
                if (this.props.selectedControl.behaviourType == 'E' && this.props.selectedControl.workflowCategory.trim() == 'CW') {
                    let dataObj = formData.reduce((obj, v, index) => ({ ...obj, [v.label]: ({ ...v, index }) }), {})
                    let ins = ['Overpayment or Underpayment', 'Workstream', 'Cause', 'Facilities / Environmental Activity', 'Location', 'Legal/Policy Breach or Exception', 'Policy', 'Finance System Name', 'Payment Value']

                    let valueToBeSet = ins
                        .map(e => dataObj[e] && dataObj[e].value && (e == "Finance System Name" ? dataObj[e].value : e == "Payment Value" ? (dataObj[e].currencyValue + " " + dataObj[e].value) : dataObj[e].value.label))
                        .filter(s => (s || '') != '')
                        .join(' - ')
            ...
            setTimeout(() => {
                this.state.formData.forEach((element, index) => {
                    if (element.presetValue) {
                        ...
                        try {
                            element.value = math.eval(formulaParts.join(" ")).toFixed(2);
                            let formData = this.state.formData;
                            this.setState({ 'formData': formData });
            ...
            triggers.forEach((trigger) => {
                if (trigger && trigger.field) {
                    ...
                    if (trigger.refValue == compareval) {
                        if (trigger.display === "hide")
                            $("#" + trigger.field + "Div").hide();
                        else
                            $("#" + trigger.field + "Div").show();
                    }
                    else {
                        if (trigger.display === "hide")
                            $("#" + trigger.field + "Div").show();
    ...
    componentDidMount() {
        if (window.DcFormRenderTriggers.length) {
            ...
            window.DcFormRenderTriggers.forEach(t => {
                ...
                const currentElement = this.state.formData.find(i => i.name === triggerField);
                ...
                if (currentElement) {
                    ...
                    const currentValue = currentElement.type === 'select'
                        // By logic, we should take select value to compare with, but by requirements comparing with label
                        ? (currentElement.value ? currentElement.value.label : currentElement.value) // for select we have an object like { label: 'label', value: 'value'}
    ...
    handleFileChange = (uploadedFiles, index) => {
        ...
        for (let i = 0; i < uploadedFiles.length; i++) {
            ...
            if (uploadedFileType === "") {
                uploadedFileType = uploadedFiles[i].name.split(".")[uploadedFiles[i].name.split(".").length - 1];
            }
            if (!(this.allowedFileTypes.indexOf(uploadedFileType) >= 0)) {
                toastr.error(uploadedFiles[i].name + " is an invalid file type. Only 'PDF / PNG / JPEG / RTF / DOCX / XLSX / TXT / MSG / ZIP / RAR /XLSB' files are allowed.");
                flag = false;
                return false;
            }
            else {
        ...
        if (flag === false) {
            ...
            let formData = this.state.formData;
            fetch(window.location.protocol + "//" + window.location.host + '/ActionCentre/UploadFiles', { // Your POST endpoint
                method: 'POST',
                headers: {
                    dataType: 'json',
                    processData: false
                ...
                response.json().then((responseData) => {
                    if (formData[index].multiple === true) {
                        formData[index].value = formData[index].value ? [...formData[index].value, ...responseData] : responseData;
    ...
    generateControl(obj, i, formRender) {
        ...
        let readOnly = formRender.props.readOnly === undefined ? false : formRender.props.readOnly;
        ...
        if (obj.conditioanalField && window.DcFormRenderTriggers.findIndex((t) => t.field === obj.name) === -1) {
        ...
        switch (obj.type) {
            case 'select':
                if (obj.parentName) {
                    ...
                    if (flag === false)
                        ...
                        return (
                            <DependentDropdown readOnly={readOnly} handleDropdownChange={formRender.handleChange} parentObjData={parentObjData} index={i} key={i} {...obj} controlStyle={controlStyle} />
                        );
                    }
                }
                else
                    return (
                        <DropDown readOnly={readOnly} handleDropdownChange={formRender.handleChange} index={i} key={i} value={obj.value ? obj.value : ""} {...obj} controlStyle={controlStyle} />
                    );
            case 'text':
                {
                    return (<TextBox readOnly={readOnly} index={i} handleInput={formRender.handleChange} key={i} {...obj} controlStyle={controlStyle} />);
                }
            case 'date': {
                if (!obj.isConverted) {
                ...
                return (
                    <DatePicker readOnly={readOnly} index={i} handleInput={formRender.handleChange} key={i} {...obj} controlStyle={controlStyle} />
                );
            }
            case 'textarea': return (
                <TextArea readOnly={readOnly} index={i} handleInput={formRender.handleChange} key={i} {...obj} controlStyle={controlStyle} />
            );
            ...
            case 'checkbox-group': return (
                <CheckBox readOnly={readOnly} handleCheckChange={formRender.handleCheckboxChange} index={i} key={i} {...obj} controlStyle={controlStyle} />
            );
            case 'radio-group': return (
                <RadioBox readOnly={readOnly} handleRadioChange={formRender.handleChange} index={i} key={i} {...obj} controlStyle={controlStyle} />
            );
            case 'number': return (
                <NumberBox readOnly={readOnly} checkDigitsNo={formRender.checkDigitsNo} handleInput={formRender.handleChange} index={i} key={i} {...obj} controlStyle={controlStyle} />
            );
            case 'file': return (
                <FileUpload readOnly={readOnly} setFilePreview={formRender.setFilePreview} removeFile={formRender.removeFile} handleInput={formRender.handleFileChange} index={i} key={i} allowedFileTypes={formRender.allowedFileTypes} {...obj} controlStyle={controlStyle} />
            );
            case 'section': return (
                <Section index={i} key={"section" + i} formRender={formRender} {...obj} controlStyle={controlStyle} />
            );
            case 'inlineGroup': return (
                <InlineGroup index={i} key={"InlineGroup" + i} formRender={formRender} {...obj} controlStyle={controlStyle} />
            );
            case 'dynamicFields': {
                ...
                return (
                    <DynamicFields readOnly={readOnly} index={i} key={"dynamicFields" + i} formRender={formRender} {...obj} controlStyle={controlStyle} />
                );
            }
            case 'page': return (
                <Page index={i} key={"page" + i} formRender={formRender} {...obj} controlStyle={controlStyle} />
            );
            case 'table': return (
                <Table index={i} key={"table" + i} formRender={formRender} {...obj} controlStyle={controlStyle} />
            );
            case 'currency':
                {
                    return (
                        <Currency readOnly={formRender.props.readOnly} index={i} key={i} value={obj.value ? (obj.value.value === 'undefine' ? obj.value : obj.value.value) : ""}
                            currencyValue={obj.currencyValue ? obj.currencyValue : ""}
                            min={formRender.props.min} max={formRender.props.max}
            ...
            case 'grid': return (
                <Grid readOnly={readOnly} index={i} key={"grid" + i} formRender={formRender} {...obj} onChange={(...args) => formRender.handleGridChange(i, ...args)} controlStyle={controlStyle} />
            );
            case 'grid2': return (
                <Grid2 readOnly={readOnly} index={i} key={"grid2" + i} formRender={formRender} {...obj} onChange={formRender.handleGrid2Change(i)} controlStyle={controlStyle} />
            );
            default:
                return null;
        }
    ...
    generateLayout(layout, formRender) {
        ...
        return layout.elements.map((element, i) => {
            ...
            let obj = null;
            let index = 0;
            if (element.type === "section" || element.type === "inlineGroup" || element.type === "dynamicFields" || element.type === "page" || element.type === "table" || element.type === "grid2") {
                obj = element;
                index = 1000000 + i;
            } else {
    ...
    generateForm() {
        ...
        } else return formRender.state.formData.map((obj, i) => {
    ...
    render() {
        ...
        return (<div>
            ...
            {
                (this.state.filePreviewSource !== '') && <div className="filePreview" onClick={this.closePreview}>
                    <div className="filePreviewContent" dangerouslySetInnerHTML={{ __html: this.state.filePreviewSource }} />
