...
export default class DatePicker extends Component {
    ...
    static findComponent = (name, formData) => formData.find(e => e.name === name) || {};
        ...
        return {
            ...
            min: currentProps.minDate ?
                (DatePicker.isDateRef.test(currentProps.minDate)
                    ? DatePicker.findComponent(currentProps.minDate, formData).value : currentProps.minDate)
                : undefined,
            max: currentProps.maxDate ?
                (DatePicker.isDateRef.test(currentProps.maxDate)
                    ? DatePicker.findComponent(currentProps.maxDate, formData).value : currentProps.maxDate)
                : undefined,
    ...
    render() {
        ...
        return (<div id={this.props.name + 'Div'} className={"form-group " + (this.props.hidden ? 'hidden' : 'shown')} style={this.props.controlStyle}>
            ...
            <input id={this.props.name + 'id'} ref={this.myRef} disabled={this.props.readOnly}
                className="datepicker  form-control"
                placeholder={this.props.placeholder}
                type="text"
                onChange={(e)=> e.target.value='' }
            />
            <div className="printDatePicker" style={{ display: 'none' }}>{this.props.value ? new Date(this.props.value).toDateString() : ""}</div>
        </div>);
    }
    ...
    componentWillReceiveProps(newProps) {
        ...
        try {
            ...
            if (newProps.format == "DD-MM-YYYY") {
                ...
                try {
                    newmin = "" + (newProps.min.getMonth() + 1) + "-" + newProps.min.getDate() + "-" + newProps.min.getFullYear() + "";

                } catch (err) {
                    if (newProps.min) {
                        var datePiecesMin = newProps.min.split("-");
                        newmin = "" + datePiecesMin[1] + "-" + datePiecesMin[0] + "-" + datePiecesMin[2] + "";
                ...
                try {
                    newmax = "" + (newProps.max.getMonth() + 1) + "-" + newProps.max.getDate() + "-" + newProps.max.getFullYear() + "";

                } catch (err) {

                    if (newProps.max) {
                        var datePiecesMax = newProps.max.split("-");
                        newmax = "" + datePiecesMax[1] + "-" + datePiecesMax[0] + "-" + datePiecesMax[2] + "";
                    }
                }
                ...
                try {
                    if (newProps.min && moment(newProps.min, 'DD-MMMM-YYYY', true).isValid()) {
                ...
                try {
                    if (newProps.max && moment(newProps.max, 'DD-MMMM-YYYY', true).isValid()) {
        ...
        if (moment(currentDatePickerformatedValue, 'DD-MMMM-YYYY', true).isValid()) {
            currentDatePickerformatedValue = moment(currentDatePicker.value, "DD-MMMM-YYYY").format("YYYY-MM-DD");
        }
        if (typeof changedDatepickerId !== 'undefined') {
            if (options.maxDate && currentDatePicker.maxDate != undefined && currentDatePicker.maxDate === changedDatepickerId) {

                id.data("DateTimePicker").maxDate(moment(options.maxDate, "YYYY-MM-DD").format("DD-MMMM-YYYY"));
            ...
            if (options.minDate && currentDatePicker.minDate != undefined && currentDatePicker.minDate === changedDatepickerId) {
                id.data("DateTimePicker").minDate(moment(options.minDate, "YYYY-MM-DD").format("DD-MMMM-YYYY"));
