...
export default class Table extends Component {
        ...
        for (let i = 0; i < rowsCount; i++) {
            ...
            for (let j = 0; j < columnsCount; j++) {
                ...
                if (obj) {
                    ...
                    children.push(
                        <td key={'LayoutTableCell_' + i + '_' + j} colSpan={colspan} rowSpan={rowspan}>
                            { this.props.formRender.generateLayout(obj, this.props.formRender) }
                        </td>);
                }
                else children.push(<td key={'LayoutTableCell_' + i + '_' + j}>&nbsp;</td>);
            }
    ...
    render() {
        ...
        if (this.props.bordered === true)
            layoutTableClass += "LayoutTableBordered";

        return (
          <div style={this.props.controlStyle}>
            {this.renderDescription()}
            <table className={layoutTableClass} id={this.props.name + 'Div'} key={this.props.name}>
                <tbody>
