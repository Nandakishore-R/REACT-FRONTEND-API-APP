    ...
    _prepareCheckBoxes() {
        if (this.props.readOnly == true) {
            return this.state.values.map((obj, ind) => {
                return (
                  <label className="p-r-15" key={ind}>
                        <input disabled type="checkbox" id={this.props.name} name={this.props.name} value={obj.value} checked={this.props.values && obj.selected} onChange={(evt) => this.props.handleCheckChange(evt.target, this.props.index)} />
            ...
            return this.state.values.map((obj, ind) => {
                return (
                  <label className="p-r-15" key={ind}>
                    <input type="checkbox" name={this.props.name} id={this.props.name}  value={obj.value} checked={this.props.values && obj.selected } onChange={(evt) => this.props.handleCheckChange(evt.target, this.props.index)} />
                    {this.renderMarkForToggle()}
                    {obj.label}
                  </label>
                );
