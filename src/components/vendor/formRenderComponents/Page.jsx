...
export default class Page extends Component {
    ...
    generatePages() {
        ...
        return this.props.pages.map((obj, i) => {
            return <div style={thumbnailsViewStyle}
                key={'LayoutPage' + i + 'Div'} id={this.props.name + '_Page_' + i}>
    ...
    componentDidMount() {
        this.pageLayoutRef.current.addEventListener('scroll', this.scrollPagesHandler);
    }

    componentWillUnmount() {
        this.pageLayoutRef.current.removeEventListener('scroll', this.scrollPagesHandler);
    }
    ...
    scrollPagesHandler = ({ target }) => {
        if (!this.__setActiveStageSilent){
            ...
            const visibleActiveIndex = Array.from(target.children[0].children).findIndex((() => {
                let totalOffsetHeight = 0 - shift;

                return ({ offsetHeight }) => scrollTop >= totalOffsetHeight && scrollTop < (totalOffsetHeight += (offsetHeight + margin));
            })());
    ...
    setActivePage = (i, scrollTo) => {
        ...
        if (scrollTo) {
            ...
            $(viewId).animate({
                scrollTop: $(viewId).scrollTop() - $(viewId).offset().top + $(pageId).offset().top - 20
            }, 500, () => this.__setActiveStageSilent = false);
        }

        $("#" + key).parent().children().removeClass("activeThumbnail");
    ...
    render() {
        ...
        return <div className="LayoutPage" id={this.props.name + 'Div'} key={this.props.name} style={this.props.controlStyle}>
            <div className="LayoutPageHeader">
                <div>{this.props.label}{this.renderDescription()}</div>
            </div>
            <div className="LayoutPageBody">
                <div className="LayoutPageThumbnails" style={{ display: this.props.multiple_pages == "Yes" ? "" : "none" }} ref={this.pageThumbnailsRef}>
                    <div>
