...
export default class DynamicFields extends Component {
    ...
    generateInlineGroup() {
        let layout = this.props.formRender.generateLayout(this.props, this.props.formRender);
        ...
        let currentLayout = layout.map((element, i) => {
            return (<div style={style} key={this.props.name + 'DivColumn' + i}>
                {element}
            </div>);
    ...
    createDynamicSectionObj(obj, appArr, newSectionArr, name = {}) {
        for (var prop in obj) {
            ...
            if (typeof obj[prop] == 'object') {
                if (prop != "cells") {
                    this.createDynamicSectionObj(obj[prop], appArr, newSectionArr, { name: "innerText" });
                }
                ...
                if (appArr.length > 0) {
                    if (name.hasOwnProperty('name') && !obj.hasOwnProperty('type')) {
                        ...
                        let newfieldname = obj.name.split('-')[0] + '-' + (new Date().getTime()) + Math.floor(Math.random() * 899 + 100);
    ...
    generateNewInlineGroup() {
        ...
        testArray.map((event, i) => {
            if (i < this.initialState.length) {
                if (event.name != undefined) {
                    var newfieldname = event.name.split('-')[0] + '-' + (new Date().getTime()) + Math.floor(Math.random() * 899 + 100);
        ...
        if (this.props.elements[0].hasOwnProperty("type") && (this.props.elements[0].type == "section" || this.props.elements[0].type == "inline" || this.props.elements[0].type == "inlineGroup")) {
            this.countSectionField = 0;
            ...
            newSectionArr.map((item, index) => {
                if (this.finalElementCount > index)                
    ...
    addFields() {
        ...
        let dynamicFieldsCreated = (this.props.elements.length - this.props.finalElements) / this.props.finalElements
        if (dynamicFieldsCreated < this.props.maxDynamicFields) {
            this.currentFieldsAdded = this.currentFieldsAdded + 1;
            this.displayData.push(<div className="LayoutInlineGroupBody" test={this.currentFieldsAdded}>{this.generateNewInlineGroup()}</div>);
            this.setState({
                showdata: this.displayData,
                currentFieldsAdded: this.currentFieldsAdded
            });
        } else {
            alert(this.userLang != 'fr' ? "Maximum Limit Reached" : "Limite maximale atteinte");
        }
    ...
    deleteFields(event, nowindex,name) {
        ...
        if (arrayIndices.length > 1) {
            this.props.elements = this.props.elements.filter((obj, index) => {
                ...
                let testArr = this.props.formRender.state.formData.some(el => el.name === obj.name);
                if (testArr)
                    return obj;
            })
            //To remove the elements from the layout once deleted.
            let layoutIndex2 = this.props.formRender.props.data.findIndex((obj) => obj.type == "layout")
            let layout = this.props.formRender.props.data[layoutIndex2].elements[0].elements ||  this.props.formRender.props.data[layoutIndex2].elements;
            if (layout) {
            ...
            let index = this.props.elements.findIndex(i=> i.name == name.replace("_control", ""))
            let extractedElements = this.props.elements.splice(index, 1);
            this.deletedFieldIndexes = [];
            this.getIndexesForDelete(extractedElements);
            arrayIndices = JSON.parse("[" + (this.deletedFieldIndexes).toString() + "]");
        }
        this.currentFieldsAdded = this.currentFieldsAdded - 1;
        this.props.formRender.state.formData = this.props.formRender.state.formData.filter((obj, index) => {
            return !arrayIndices.includes(index);
        })
        this.props.formRender.props.data = this.props.formRender.props.data.filter((obj, index) => {
            return !arrayIndices.includes(index);
        })
            ...
            if (element.props.children !== null) {
                ...
                if (currentIndex % this.props.finalElements == 0) {
                    ...
                    if (currentIndex > this.props.finalElements) {
                        tempfields.push(<button className="btn btn-danger btn-sm deleteDynamic" style={{ maxHeight: '3vh', marginTop: !this.props.requireLabel ? '' : '3vh', display: this.props.readOnly ? 'none' : '' }} value={tempIndex} name={element.props.children.props.name+"_control"} onClick={(e) => {
                            this.deleteFields(e.target.value, this.currentFieldsAdded, e.target.name);
                        }}>X</button>);
                    } else {
                        tempfields.push(<button className="btn btn-danger btn-sm deleteDynamic" style={{ maxHeight: '3vh', visibility: 'hidden', display: this.props.readOnly ? 'none' : '' }} value={tempIndex} name={element.props.children.props.name + "_control"} onClick={(e) => {
                            this.deleteFields(e.target.value, this.currentFieldsAdded, e.target.name);
                        }}>X</button>);
                       
                    }

                    if (currentIndex > this.props.finalElements && !this.props.requireLabel) {
                        mainLayout.push(<div className="LayoutInlineGroupBody showLabel" style={{ alignItems: 'center', marginBottom: '2vh' }}> {tempfields}</div>);
                    }
                    else {
                        
                        mainLayout.push(<div className="LayoutInlineGroupBody" style={{ alignItems: 'center', marginBottom: '2vh' }}>{tempfields}</div>);
                    }
                    deleteButton = [];
                    tempIndex = [];
                    tempfields = [];
    ...
    render() {
        ...
        return <div className={layoutInlineGroupClass} key={this.props.name} id={this.props.name + 'Div'} style={this.props.controlStyle}>
            {this.renderDescription()}
            {this.generateDynamicFields()}
            <div style={{ display: 'flex', justifyContent: 'end' }}>
                <button className="btn btn-primary btn-sm" style={{ display: (this.props.readOnly || this.props.maxDynamicFields <= 0 || !this.props.maxDynamicFields) ? 'none' : '' }} onClick={() => {
                    this.addFields();
                }}>{this.userLang != 'fr' ? "Add More" : "Ajouter"}</button>
            </div>
        </div>;
    }
}
