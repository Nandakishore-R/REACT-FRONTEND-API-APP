...
function StagesList(props) {
    ...
    const { stageList, setStageList, numberOfApprovalStages, setNumberOfApprovalStages, currentLanguage, translatorObject } = props;
    ...
    const columns = [
        ...
        },
        {
            ...
            >
                {options && options.map(value => (
                    <Option value={value.label} key={value.label} isuser={value.isUser}>
                        <Tooltip title={value.label} key={value.label} isuser={value.isUser}>
        ...
        },
        {
            ...
            width: "20%",
            key: "3",
            render: (text, record) => (<Input type="text" defaultValue={record.Header} onChange={(e) => {
                ...
                <Col>
                    ...
                    {/*/>*/}
                    {
                        ...
                        <img
                            ...
                            onClick={(e) => {
                                e.stopPropagation();
                                let arrayList = stageList.filter((item, index) => stageList.indexOf(item) != indexVal);
        ...
        return (
            <Fragment>
                ...
                >
                    ...
                    <Collapse collapsible="icon" bordered={false} defaultActiveKey={[nodeIndex.toString()]} >
                        <Panel key={stage.index} header={<PanelHeader stage={stage} />}>
                            <Row className="stageFlexBetween mtb2" >
                                <Col className="displayFlex">
                                    <label className="label-stages" style={{ fontWeight: "bold", color: "black" }}>{t('Label_StageName')} <i className="fa fa-circle asterisk-dot"></i></label>
                                    <Input type="text"
                                        placeholder={t('Label_StageName')} defaultValue={stageValue.StageName} className="stageNameInput" onChange={(e) => {
                                            let value = e.target.value;
                            ...
                            <Row className="stageFlexBetween mtb2">
                                <Col className="stageWidth3">
                                    <label className="label-stages" style={{ fontWeight: "bold", color: "black" }}>{t('Label_ReviewWithin')}</label>
                                    <InputNumber defaultValue={stageValue.ReviewBy} min={0} onChange={(e) => {
    ...
    return (
        <div>
            <span id="formBuilder-stages"
                style={modaldisplayshow ? { visibility: "visible", zIndex: "1" } : { visibility: "hidden", zIndex: "1" }} >
                <div>
                    <Row className="navbar-stageform">
                        ...
                        <Col span={19}></Col>
                        <Col span={3}>
                            <Button onClick={savestagesform} className="save-stages-form">SAVE AND CLOSE</Button>
                        </Col>
                    </Row>
                    <div className="" id="formDesignerAreaStage">
                    </div> <div className="" id="loading" ></div></div>
            </span>
